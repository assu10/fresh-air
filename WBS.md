...
| # | 작업 | 산출물 | 상태 |
|---|------|--------|------|
| 4.1 | 크론 로직 테스트 작성 (Red) | `__tests__/api/cron/check.test.ts` | ✅ |
| 4.2 | 크론 핵심 알림 로직 구현 (Green) | `app/api/cron/check/route.ts` | ✅ |
| 4.3 | 리팩토링 + 테스트 통과 확인 | — | ✅ |
| 4.4 | TASK 문서 작성 | `docs/work/TASK-20260503-크론체크엔드포인트/` | ✅ |
...
## Phase 7: PWA 강화 — 모바일 설치 경험 개선

| # | 작업 | 산출물 | 상태 |
|---|------|--------|------|
| 7.1 | 아이콘 에셋 생성 (sharp) | `public/icons/`, `public/apple-touch-icon.png` | ⬜ |
| 7.2 | manifest.ts 업데이트 | `app/manifest.ts` | ⬜ |
| 7.3 | useInstallPrompt 훅 (TDD) | `lib/hooks/useInstallPrompt.ts` | ⬜ |
| 7.4 | InstallPrompt 컴포넌트 (TDD) | `components/InstallPrompt.tsx` | ⬜ |
| 7.5 | useNotification iOS 지원 강화 (TDD) | `lib/hooks/useNotification.ts` | ⬜ |
| 7.6 | NotificationToggle requiresInstall 추가 | `components/NotificationToggle.tsx` | ⬜ |
| 7.7 | Service Worker 캐싱 전략 추가 | `public/sw.js` | ⬜ |
| 7.8 | page.tsx InstallPrompt 통합 | `app/page.tsx` | ⬜ |
